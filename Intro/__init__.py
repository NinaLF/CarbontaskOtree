    ...
    def vars_for_template(player: Player):
        ...
        if(player.session.config['language']=="en"):   
            if(player.participant.order_tasks == 1):
                ...
                stringOrder = "http://psbc.otree.psychologie.unibas.ch/join/tepedafe?participant_label=" + player.participant.label 
            else:
        ...
        if(player.session.config['language']=="zh_hans"):   
            if(player.participant.order_tasks == 1):
                #insert session link for Order_Nina_Jessi_Zahra
                stringOrder = "http://psbc.otree.psychologie.unibas.ch/join/lodezesa?participant_label=" + player.participant.label
            elif(player.participant.order_tasks == 2):
                #insert session link for Order_Jessi_Zahra_Nina
                stringOrder = "http://psbc.otree.psychologie.unibas.ch/join/pirirobu?participant_label=" + player.participant.label
            else:
                #insert session link for Order_Zahra_Nina_Jessi
                stringOrder = "http://psbc.otree.psychologie.unibas.ch/join/limesabo?participant_label=" + player.participant.label
